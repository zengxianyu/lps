...
class PriorFolder(BaseFolder):
    def __init__(self, root, prior_dir, size=256, crop=None, rotate=None, flip=False,
                 mean=None, std=None):
        super(PriorFolder, self).__init__(root, crop=crop, rotate=rotate, flip=flip, mean=mean, std=std)
        self.size = size
        self.pr_filenames = [os.path.join(prior_dir, name+'.png') for name in self.names]
    ...
    def __getitem__(self, index):
        ...
        img, gt, pr = [_img.resize((self.size, self.size)) for _img in [img, gt, pr]]
...
class Folder(BaseFolder):
    def __init__(self, root, scales=[256], crop=None, rotate=None, flip=False,
                 mean=None, std=None):
        super(Folder, self).__init__(root, crop=crop, rotate=rotate, flip=flip, mean=mean, std=std)
        self.scales = scales
